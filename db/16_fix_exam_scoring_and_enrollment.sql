-- ==========================================
-- EXAM SCORING & ENROLLMENT SYNC FIX
-- ==========================================
-- Date: 05 Feb 2026
-- Description: 
-- 1. Updates submit_exam to automatically mark course as COMPLETED in enrollments when exam is passed.
-- 2. Ensures strict validation of questions (denominator = total DB questions).
-- 3. Handles dynamic passing score from exams table.
-- ==========================================
...
BEGIN
    ...
    -- 8. UPDATE ENROLLMENT (CRITICAL SYNC)
    -- If passed, mark course as completed
    IF v_passed THEN
        UPDATE public.enrollments
        SET is_completed = TRUE,
            progress_percent = 100,
            -- completed_at = now() -- Assuming column exists, if not safe to omit or use updated_at
            enrolled_at = COALESCE(enrolled_at, now()) -- Ensure enrolled_at is kept
        WHERE user_id = v_user_id AND course_id = p_course_id;
    END IF;
